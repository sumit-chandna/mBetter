...
<%-- <template:errorSpanField path="${path}"> --%>
        <c:if test="${showLabel != false}">
            <label class="control-label ${labelCSS}" for="${idKey}"> <spring:message code="${labelKey}" text="${defaultText}"/> <c:if
                    test="${mandatory != null && mandatory == true}">
                    <span class="mandatory"> <spring:message code="login.required" var="loginRequiredText" /><span style="color:red">*</span>
                    </span>
            ...
            <form:select id="${idKey}" path="${path}" cssClass="${selectCSSClass}" tabindex="${tabindex}" disabled="${disabled}">
                <c:if test="${skipBlank == null || skipBlank == false}">
                    <option value="" disabled="disabled" ${empty selectedValue ? 'selected="selected"' : ''}>
                        <spring:message code='${skipBlankMessageKey}' />
                    </option>
                </c:if>
                <form:options items="${items}" itemValue="${not empty itemValue ? itemValue :'dropDownValue'}"
                    itemLabel="${not empty itemLabel ? itemLabel :'dropDownText'}" />
            </form:select>
        </div>
